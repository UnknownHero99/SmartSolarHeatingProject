  ...
  public:
    ...
    String updateTemp() {
      if (this->enabled) {
        ...
        float temporary = sensor->getTempCByIndex(0);
        for (int i = 0; i < 3 && temporary == DEVICE_DISCONNECTED_C; i++)
          temporary = sensor->getTempCByIndex(0);

        this->sensorTemperature = temporary;
        if (this->sensorTemperature == DEVICE_DISCONNECTED_C) return "N/A";
    ...
    String temp() {
      if (this->enabled && this->sensorTemperature != DEVICE_DISCONNECTED_C)
