...
void TempHandler() {
	if (autoMode) {
		if ((collectorSensor.temp() - boilerSensor.temp() >= settingsMinTempDifference && collectorSensor.temp() >= settingsMinTempCollector && boilerSensor.temp() < settingsMaxTempBoiler && !mainPump.isOperating()) || (collectorSensor.temp() > settingsMaxTempCollector && boilerSensor.temp() < settingsMaxTempBoiler && !mainPump.isOperating()))
		{
			mainPump.on();
		}

		if ((collectorSensor.temp() - boilerSensor.temp() < settingsMinTempDifference && collectorSensor.temp() < settingsMaxTempCollector && mainPump.isOperating()) || (boilerSensor.temp() >= settingsMaxTempBoiler && mainPump.isOperating()) || (collectorSensor.temp() < settingsMinTempCollector && mainPump.isOperating())) {
			mainPump.off();
		}
	}
	else {
		if ((collectorSensor.temp() > settingsMaxTempCollector && boilerSensor.temp() < settingsMaxTempBoiler && !mainPump.isOperating())) {

			mainPump.on();
		}

		if ((boilerSensor.temp() >= settingsMaxTempBoiler && mainPump.isOperating())) {
			mainPump.off();
		}
	}
}
...
void ledHandler() {
	...
	if (collectorSensor.temp() == -127) {
		setColor(0, 0, 255);
		Serial.println("Problem with collector temp sensor");
	}

	if (boilerSensor.temp() == -127) {
		setColor(0, 0, 255);
	...
	if (boilerSensor.temp() > settingsMaxTempBoiler || collectorSensor.temp() > settingsMaxTempCollector) {
		setColor(255, 0, 0);
